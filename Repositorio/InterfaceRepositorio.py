...
class InterfaceRepositorio(Generic[T]):
    ...
    def __init__(self):
        ...
        theClass = get_args(self.__orig_bases__[0])  # nombre relacionado con el objeto que entro
        self.coleccion = theClass[0].__name__.lower()  # guarde el nombre de clase en minuscula

    def loadFileConfig(self):
        with open('config.json') as f:  # with es un contexto basicamente se establese con una configuracion inicial y una finalizacion para recuperar los valores
            data = json.load(f)
        return data

    def save(self, item: T):
        laColeccion = self.baseDatos[self.coleccion]  # busca la coleccion estudiante
        elId = ""  # crea el id vacio
        item = self.transformRefs(item)  # utiliza una funcion de su propia clase- transforma
        if hasattr(item, "_id") and item._id != "":
            ...
            _id = laColeccion.insert_one(item.__dict__)  # guarda el dato
            elId = _id.inserted_id.__str__()

        x = laColeccion.find_one({"_id": ObjectId(elId)})  # guardar el objeto creado como object id
        x["_id"] = x["_id"].__str__()  # combierte el mismo dato a str
    ...
    def findAll(self):  # busca por filtro
        laColeccion = self.baseDatos[self.coleccion]
        data = []
